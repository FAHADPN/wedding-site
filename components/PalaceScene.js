...
function Layer({ src, z, f, order, pos, cover, float, objPos = 'center bottom', alt = '', filter, mask }) {
  return (
    ...
    >
      {desktop ? (
        ...
        <>
          ...
          <div className="layer-fade" aria-hidden="true" style={{ position: 'absolute', zIndex: 0, left: '50%', bottom: '4%', width: '80%', height: '85%', transform: 'translateX(-50%)', pointerEvents: 'none', background: 'radial-gradient(ellipse at 50% 64%, rgba(214,150,70,0.22) 0%, rgba(150,90,40,0.1) 38%, transparent 68%)', filter: 'blur(12px)' }} />
          {/* sized to viewport height so its top always tucks behind the arch across screen sizes */}
          <Layer src={`${SCENE}/palace.webp`} z={1} f={10} order={1} filter="blur(0.8px) saturate(0.96) brightness(0.97)" mask="linear-gradient(180deg, #000 0%, #000 84%, transparent 100%)" pos={{ left: '50%', bottom: '6%', width: 'min(86%, 132dvh)', transform: 'translateX(-50%)' }} />
